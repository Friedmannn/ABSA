...
class Gaussian(ContinuousDistribution):
    ...
    def __init__(self, mean, stdev, points=[]):
        self.mean = mean
        self.stdev = stdev
    ...
    def param_estimate(self):
        numPoints = float(len(self.points))
    ...
    def clear(self):
        '''
        clear data points for memory saving
        '''
        self.points = []
...
class Multinomial(DiscreteDistribution):
    ...
    def __init__(self, categoryCounts=None, smoothingFactor = 1.0):
        if categoryCounts:
            self.categoryCounts = categoryCounts
            self.numPoints = float(sum(categoryCounts.values()))
            self.numCategories = float(len(categoryCounts))
        else:
            self.categoryCounts = collections.Counter()
            self.numPoints = 0.0
            self.numCategories = 0.0
        self.smoothingFactor = float(smoothingFactor)

    
    def add_point(self, point):
        self.categoryCounts[point] += 1
        self.numPoints += 1
    

    def param_estimate(self):
        self.numCategories = float(len(self.categoryCounts))


    def probability(self, value):
        if not value in self.categoryCounts:
            #return 0.0
            self.categoryCounts[value] = 1
